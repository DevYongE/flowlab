...
const Sidebar: React.FC<SidebarProps> = ({ isMini }) => {
  ...
  return (
    <div className={`h-screen bg-gray-900 text-white flex flex-col justify-between shadow-lg transition-all duration-300 ${isMini ? 'w-20 p-2' : 'w-64 p-6'}`}>
      <div>
        ...
        {!isMini && (
          <div className="mt-6 text-xs text-gray-400">
            👤 로그인 사용자: <span className="font-semibold">{user?.name || user?.id || '알 수 없음'}</span>
            {user?.position_name && (
        ...
        <ul className={`space-y-3 ${isMini ? 'text-xl mt-8' : 'text-sm mt-6'}`}>
          {menu.map((item) => (
            <li key={item.label} className="flex justify-center">
              <Link to={item.to} className="flex items-center gap-4 hover:underline">
                <span>{item.icon}</span>
