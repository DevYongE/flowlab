...
const AdminUserPage = () => {
  ...
  useEffect(() => {
    if (showEdit && selectedUser) {
      ...
      setEditDepartmentId(selectedUser.department || '');
      setSelectedCompanyCode(selectedUser.company_code || '');
    }
  ...
  const getPositionName = (code: string) => {
    const pos = positions.find(p => String(p.position_code) === String(code));
    return pos ? pos.name : code;
  };

  const getRoleName = (code: string) => {
    const role = roles.find(r => String(r.role_code) === String(code));
  ...
  const handleOpen = (type: string, user: any) => {
    ...
    if (type === 'edit') {
      ...
      setEditDepartmentId(user.department || '');
      setEditPositionCode(user.position_code || '');
      setShowEdit(true);
    }
                ...
                <tbody>
                  {filteredUsers.length === 0 ? (
                    ...
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="py-2 px-3 text-center">
                        {getPositionName(user.position_code)}
                      </td>
                      <td className="py-2 px-3 text-center">{getRoleName(user.role_code)}</td>
            ...
            {showDetail && selectedUser && (
              <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
                <div className="bg-white p-6 rounded-lg shadow-lg min-w-[350px] space-y-4">
                  ...
                  <div>부서: {getDepartmentName(selectedUser.department)}</div>
                  <div>직급: {getPositionName(selectedUser.position_code)}</div>
                  <div>권한: {getRoleName(selectedUser.role_code)}</div>
                ...
                <form className="bg-white p-6 rounded-lg shadow-lg min-w-[350px] space-y-4" onSubmit={async (e) => {
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      // 기업 변경 시 부서 및 직급 목록 초기화 또는 재조정 로직 필요
                      setEditDepartmentId(''); // 선택된 부서 초기화
                      setEditPositionCode(''); // 선택된 직급 초기화
                    }}
                  >
            ...
            {showDept && selectedUser && (
              <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
                <form className="bg-white p-6 rounded-lg shadow-lg min-w-[350px] space-y-4" onSubmit={async (e) => {
                  ...
                  const department = (form.elements.namedItem('department') as HTMLSelectElement).value;
                  try {
                    await axios.patch(`/users/${selectedUser.id}/department`, { department });
                    await fetchUsers();
                    handleClose();
                  } catch (err) {
                    alert('부서 변경 실패');
            ...
            {showPosition && selectedUser && (
              <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
                <form className="bg-white p-6 rounded-lg shadow-lg min-w-[350px] space-y-4" onSubmit={async (e) => {
                  ...
                  <select name="position_code" className="w-full border rounded p-2" value={editPositionCode} onChange={e => setEditPositionCode(e.target.value)}>
                    <option value="">직급 선택</option>
