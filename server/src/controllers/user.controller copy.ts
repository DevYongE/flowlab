...
export const getUsers = async (req: Request, res: Response) => {
  try {
    ...
    let query = `
      SELECT u.id, u.name, u.email, d.department_name, p.name AS position_name, r.name AS role_name
      FROM users u
...
export const getUserProfile = async (req: Request, res: Response) => {
  try {
    ...
    const [user]: any = await sequelize.query(
      `SELECT u.id, u.name, u.email, u.user_code, u.birth, u.join_date,
              c.company_name, d.department_name, p.name as position_name, r.name as role_name
       FROM users u
    ...
    console.error('❌ 사용자 프로필 조회 에러:', err);
    res.status(500).json({ message: '서버 오류가 발생했습니다.' });
  }
};
